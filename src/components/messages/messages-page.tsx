...
function MessageThread({
  ...
  // Opening a conversation marks it read server-side (the messages endpoint
  // updates last_read_at). Once messages have loaded, refresh the local unread
  // state so the bell/sidebar badges update immediately instead of waiting for
  // the next poll.
  const markedRef = useRef<number | null>(null);
  useEffect(() => {
    if (!data) return;
    if (markedRef.current === conversationId) return; // once per open
    markedRef.current = conversationId;
    qc.invalidateQueries({ queryKey: ["conversations"] });
    messagesApi.unreadCount().then(setUnread).catch(() => {});
  }, [data, conversationId, qc, setUnread]);
