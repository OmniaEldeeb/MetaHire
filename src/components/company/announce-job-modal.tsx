      ...
      >
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-muted">Visibility</p>
            {[
              { value: "public", label: "Public", desc: "Anyone can see this post (recommended)" },
              { value: "connections", label: "Connections", desc: "Only your followers" },
            ].map((opt) => (
              <label
                key={opt.value}
                className={`flex cursor-pointer items-start gap-3 rounded-xl border px-3.5 py-2.5 transition-colors ${
                  visibility === opt.value ? "border-brand bg-brand-soft/40" : "border-line hover:border-line2"
                }`}
              >
                <input
                  type="radio"
                  name="announce-visibility"
                  value={opt.value}
