...
export function JobChatbot() {
  ...
  const confirm = async () => {
    ...
    setConfirming(true);
    try {
      const res = await companyJobsApi.confirmChatbotJob({
        conversation_id: convId,
        job_data: jobData,
      });
      ...
      if (newJob?.id) {
        // Offer to announce it to the feed (POST /api/posts flow).
        setAnnounceJob({ id: newJob.id, title: newJob.title ?? "this role" });
      } else {
  ...
  return (
    ...
    {announceJob && (
      <AnnounceJobModal
        ...
        onClose={() => router.push("/company/jobs")}
      />
    )}
    </>
  );
