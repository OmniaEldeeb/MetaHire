...
import {
  ...
  ChevronRight, MapPin, Building2, CalendarRange, Send, FileText,
} from "lucide-react";
      ...
      >
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-5">
          ...
          <Link
            ...
            className="flex items-center justify-center gap-1.5 rounded-2xl border border-line bg-surface px-4 py-3 text-sm font-semibold text-brand transition-colors hover:border-brand"
          >
...
export function ApplicantsList({ jobId }: { jobId: number }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["company-applications", jobId, page, statusFilter],
    queryFn: () =>
      companyJobsApi.applications(jobId, {
        page,
  ...
  const autoInvite = async () => {
    ...
    try {
      const res = await companyJobsApi.autoInvite(jobId);
      qc.invalidateQueries({ queryKey: ["company-applications", jobId] });
      const count = res?.count ?? 0;
      toast(
        count > 0
          ? { kind: "success", title: `Invited ${count} candidate${count !== 1 ? "s" : ""}` }
          : { kind: "info", title: "No candidates matched", message: "Adjust the auto-invite thresholds or wait for more applicants." },
      );
    } catch {
        ...
        {isLoading ? (
          ...
          <div className="grid place-items-center gap-3 py-20 text-center">
            ...
            <p className="text-sm text-muted">
              {statusFilter ? `No applicants with status "${APPLICATION_STATUS_LABELS[statusFilter]}".` : "No applicants yet."}
            </p>
          </div>
        ) : (
          <>
