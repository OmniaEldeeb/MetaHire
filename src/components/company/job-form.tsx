...
function defaults(job?: Job): FormValues {
  return {
    ...
    auto_invite_enabled: job?.auto_invite_settings?.enabled ?? false,
    auto_ai_invite_count: String(job?.auto_invite_settings?.ai_count ?? 5),
    auto_final_invite_count: String(job?.auto_invite_settings?.final_count ?? 3),
    min_cv_score: String(job?.auto_invite_settings?.min_cv_score ?? 70),
    min_ai_score: String(job?.auto_invite_settings?.min_ai_score ?? 60),
    focus_criteria: (job?.focus_criteria ?? []).join(", "),
    skills: job?.skills ?? [],
  };
}
...
export function JobForm({ job }: { job?: Job }) {
  ...
  const [announceJob, setAnnounceJob] = useState<{ id: number; title: string } | null>(null);
  ...
  async function onSubmit(v: FormValues) {
    ...
    try {
      if (isEdit) {
        ...
        const newId = (newJob as Job).id;
        if (newId) {
          // Offer to announce it to the feed (its own POST /api/posts flow).
          setAnnounceJob({ id: newId, title: v.title });
        } else {
  ...
  return (
    ...
    {announceJob && (
      <AnnounceJobModal
        ...
        jobTitle={announceJob.title}
        onClose={() => router.push(`/company/jobs/${announceJob.id}`)}
      />
    )}
    </>
  );
