...
export function CompanyTeam() {
  const { canInviteMembers, canRemoveMembers, canUpdateMemberRole, canCancelInvitation, canLeaveCompany } = useCompanyAccess();
            ...
            {(members.data ?? []).map((m) => (
              <li key={m.id} className="flex items-center gap-3 py-3">
                ...
                {canUpdateMemberRole ? (
                  ...
                  <span className="rounded-lg border border-line px-2 py-1 text-xs font-medium capitalize text-muted">
                    {m.role}
                  </span>
                )}
                {canRemoveMembers && (
