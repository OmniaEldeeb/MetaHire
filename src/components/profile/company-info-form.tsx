...
export function CompanyInfoForm({ company }: { company: Company }) {
  const { canEditProfile } = useCompanyAccess();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
      ...
      {!canEditProfile && (
        <p className="rounded-xl border border-line bg-elevated px-4 py-2.5 text-xs text-muted">
          Only the company owner can edit the company profile — you have view access.
        </p>
      )}

      <fieldset disabled={!canEditProfile} className="m-0 min-w-0 space-y-6 border-0 p-0 disabled:opacity-70">
      {/* Cover + Logo */}
