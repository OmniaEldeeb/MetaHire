...
// Notification types that are sent to company members (owner + HR). These must
// open company views regardless of the recipient's role, because HR members
// are candidate-role accounts.
const COMPANY_NOTIF_TYPES = new Set(["new_application", "candidate_interview_completed"]);
      ...
      {!loaded ? (
        ...
        <ul className={cn("divide-y divide-line", compact && "max-h-96 overflow-auto")}>
          {shown.map((n) => {
            ...
            // Route by the notification, not the user's role. HR members are
            // candidate-role users, so company-facing notifications must still
            // open company views. Company-side notifications always carry
            // candidate_name / job_title in their data (new_application,
            // candidate_interview_completed); candidate-facing ones never do —
            // so that data shape is the reliable signal even if the stored
            // `type` string differs.
            const d = n.data ?? {};
            const companyView =
              COMPANY_NOTIF_TYPES.has(n.type) ||
              Boolean(d.candidate_name) ||
              role === "company";
            let href: string | undefined;
            if (d.application_id) href = companyView ? "/company/applications" : "/applications";
            else if (d.job_id) href = companyView ? `/company/jobs/${d.job_id}` : `/jobs/${d.job_id}`;
