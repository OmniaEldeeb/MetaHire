...
export default function CompanyJobDetailPage({ params }: { params: { id: string } }) {
  ...
  const stats = data && !("id" in (data as object)) ? (data as { stats?: Record<string, number> }).stats : undefined;
      ...
      {stats && Object.keys(stats).length > 0 && (
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {Object.entries(stats).map(([k, v]) => (
            <div key={k} className="rounded-2xl border border-line bg-surface px-4 py-3">
              <p className="text-xl font-extrabold tracking-tight">{v}</p>
              <p className="mt-0.5 text-xs text-muted">
                {APPLICATION_STATUS_LABELS[k as ApplicationStatus] ?? k.replace(/_/g, " ")}
              </p>
            </div>
          ))}
        </div>
